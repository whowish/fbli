...
module Validator
  ...
  #      puts @__fields_to_validate_uniqueness.inspect
      ...
      def validate(all_params, controller)
        ...
        @__fields_to_validate.each_pair { |key,list|
          ...
          list.each { |func_with_msg|
            ...
            if result == false
              this_error_messages.push( (func_with_msg[:m] ? func_with_msg[:m] : word_for(key, func_with_msg[:f].name, controller.whowish_word_config.locale) ) )
            end
          }
      ...
      def validate!(all_params, controller)
        ...
        errors = validate(all_params, controller)
        
        if errors.length > 0
          raise LogicException, errors  
        end
        
      end
      
      
      def word_for(field, func_name, locale)
        WhowishWord.word_for(self.name.sub("Validator","").underscore,"#{field}_#{func_name}", locale)
      end
